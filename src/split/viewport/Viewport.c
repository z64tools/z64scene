...
static void Viewport_GizmoSelection(Editor* editor, Viewport* this, Split* split) {
	...
	if (!split->cursorInSplit || editor->nano.state.blockElemInput)
		return;
...
Rect GetRect (void* pass) {
	Split* split = pass;
	
	return split->rect;
};

Vec2f GetPoint (void* pass) {
	Split* split = pass;
	
	return Math_Vec2f_New(UnfoldVec2(split->cursorPos));
};

void Viewport_Init(Editor* editor, Viewport* this, Split* split) {
	
	View_Init(&this->view, &editor->input, split, GetRect, GetPoint);
	
	Element_SetName(&this->buttonResetCam, "Reset");
	Element_SetName(&this->buttonViewSmooth, "Smooth");
	
	Element_Button_SetProperties(&this->buttonResetCam, 0, 0);
	Element_Button_SetProperties(&this->buttonViewSmooth, true, !this->view.noSmooth);
	Element_Slider_SetParams(&this->sliderCamFov, 5, 175, "f32");
	Element_SetName(&this->sliderCamFov, "FOV");
...
void Viewport_Update(Editor* editor, Viewport* this, Split* split) {
	...
	if (!Gizmo_IsBusy(gizmo) && split->cursorInSplit && !nano->state.blockElemInput) {
	...
	if ((this->view.cameraControl && editor->input.cursor.clickAny.hold) || (gizmo->lock.state && gizmo->activeView == &this->view)) {
		...
		if (cursor->pos.x < xMin || cursor->pos.x > xMax)
			Input_SetMousePos(&editor->input, wrap(cursor->pos.x, xMin, xMax), MOUSE_KEEP_AXIS);
		
		if (cursor->pos.y < yMin || cursor->pos.y > yMax)
			Input_SetMousePos(&editor->input, MOUSE_KEEP_AXIS, wrap(cursor->pos.y, yMin, yMax));
	}
	
	EnvLights* envSettings = Arli_At(&header->envList, header->envList.cur);
