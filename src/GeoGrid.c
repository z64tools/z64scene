			...
			for (i = 0; i < 4; i++) {
				...
				for (s32 j = 0; (1 << j) <= EDGE_STICK_B; j++) {
					if (split->edge[i]->state & (1 << j)) {
						if (t++ == 0) {
							strcpy(buffer, stringEdgeState[j]);
						} else {
							strcat(buffer, "|");
							strcat(buffer, stringEdgeState[j]);
						}
					}
				}
		...
		nvgFill(vg);
			...
			nvgFillPaint(
				...
				nvgRadialGradient(
					...
					0,
					800,
					Theme_GetColor(THEME_HEADER, 255),
					Theme_GetColor(THEME_SHADOW, 255)
				)
			);
	...
	for (s32 i = 0; i < 2; i++) {
		...
		nvgBeginFrame(geoCtx->vg, geoCtx->bar[i].rect.w, geoCtx->bar[i].rect.h, 1.0f); {
			...
			if (i == 1) {
				...
				nvgFillColor(geoCtx->vg, Theme_GetColor(THEME_SHADOW, 255));
